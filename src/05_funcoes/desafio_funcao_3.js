const nota1 = 7.8;
const nota2 = 3.1;
const nota3 = 6.7;

function minimo(n1, n2) {
  return n1 <= n2 ? n1 : n2;
}

// 1 primeira forma de ser feito - 2 notas
console.log(minimo(nota1, nota2));

// 2 segunda forma de ser feito - 3 notas
console.log(minimo(nota1, minimo(nota2, nota3)));

// 3 terceira formma de ser feito - 3 notas
const menor2e3 = minimo(nota2, nota3);
console.log(minimo(nota1, menor2e3));

function media(n1, n2, n3) {
  const menorNota = minimo(n1, minimo(n2, n3));

  if (menorNota === n1) {
    return n2 + n3 / 2;
  } else if (menorNota === n2) {
    return n1 + n3 / 2;
  } else {
    return n1 + n2 / 2;
  }
}

console.log(media(nota1, nota2, nota3));

function mediaParaStatus(media) {
  if (media >= 7) {
    return 'Aprovado';
  } else if (media >= 4) {
    return 'Recuperação';
  } else {
    return 'Reprovado';
  }
}

const mediaFinal = media(nota1, nota2, nota3);
const statusFinal = mediaParaStatus(mediaFinal);
console.log(`\nO resultado final do aluno é ${statusFinal}!`);
